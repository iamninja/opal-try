...
(function($opal) {
  ...
  (function($base, $super) {
    ...
    def.$draw_canvas = function() {
      ...
      x = x['$+']((($b = $scope.CELL_WIDTH) == null ? $opal.cm('CELL_WIDTH') : $b));};
      y = 0.5;
      while (!(y['$>='](self.$height()))) {
      self.$context().moveTo(0, y);
      self.$context().lineTo(self.$width(), y);
      y = y['$+']((($b = $scope.CELL_HEIGHT) == null ? $opal.cm('CELL_HEIGHT') : $b));};
      self.$context().strokeStyle = "#bbb";
      return self.$context().stroke();
    };

    return (def.$canvas_id = function() {
      var self = this;
